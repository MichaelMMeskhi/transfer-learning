function [acc_lst, std_lst] = main(Xsrc, Ysrc, targetData, optionsTJM, optionsJDA)
    acc_lst = zeros(1,10);
    std_lst = zeros(1,10);
    acc_lst_GFK = zeros(1,10);
    acc_lst_TJM = zeros(1,10);
    acc_lst_JDA = zeros(1,10);
    acc_lst_JGSA = zeros(1,10);
...
function [train_samples, test_samples] = randomSampling(data)
    samples = cell(10, 1);
    nSample = size(data,1) / 2 + 100;
    
    for j = 1:10
        data = data(randperm(size(data, 1)), :);
        rndIDX = randi(size(data, 1), nSample, 1);
        dataSample  = data(rndIDX, :);
        train_samples{j} = dataSample(1:100, :); 
        test_samples{j} = dataSample(100:end, :)
    end
end